...
export class Metadata {
  ...
  public readonly values = { loaded: false, width: 0, height: 0, size: 0, duration: 0, type: '' };
  ...
  public async load(): Promise<Metadata> {
    ...
    const { data } = await $fetch.get(`video_metadata?url=${this.url}`, { baseUrl: environment.apiUrlV1 });
    if (!data) return this;
    const metadata = data.metadata;
    const { x, y, filesize, duration, mime_type } = metadata;
    Object.assign(this.values, { loaded: true, width: x, height: y, size: filesize, duration, type: mime_type });
    is.affirmative(this.isQuicktime()).mapRight(() => (this.values.type = 'video/mp4'));
    return this;
  }
