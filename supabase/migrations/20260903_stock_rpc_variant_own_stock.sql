-- Stock por VARIANTE (sin talla). Hasta ahora una variante sin talla caía al
-- stock a nivel de PRODUCTO (compartido entre todas las variantes). Ahora, si la
-- variante lleva su propio `stock` en el jsonb, se descuenta/repone ESE stock.
-- Retrocompatible: variantes sin `stock` propio siguen cayendo al stock del
-- producto (comportamiento previo). Pedido de Distribuidora moto Fox.
-- SECURITY DEFINER (igual que la versión anterior).
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Variante CON talla: stock por talla de la variante.
    IF v_variant_id IS NOT NULL AND v_size IS NOT NULL THEN
    ...
    -- Variante SIN talla: descuenta el stock PROPIO de la variante cuando lo
    -- lleva; si no, cae al stock del producto (comportamiento previo).
    IF v_variant_id IS NOT NULL AND v_size IS NULL THEN
      v_variants := v_row.variants; v_var_idx := -1;
      FOR i IN 0 .. COALESCE(jsonb_array_length(v_variants), 0) - 1 LOOP
        IF (v_variants->i->>'id') = v_variant_id THEN v_var_idx := i; EXIT; END IF;
      END LOOP;
      IF v_var_idx >= 0
         AND v_variants->v_var_idx->'stock' IS NOT NULL
         AND jsonb_typeof(v_variants->v_var_idx->'stock') <> 'null' THEN
        v_var_stock := GREATEST(0, (v_variants->v_var_idx->>'stock')::NUMERIC - v_quantity);
        v_variants := jsonb_set(v_variants, ARRAY[v_var_idx::TEXT, 'stock'], to_jsonb(v_var_stock));
        UPDATE products SET variants = v_variants WHERE id = v_product_id;
        v_updated := v_updated + 1;
        CONTINUE;
      END IF;
      -- variante sin stock propio → cae al stock del producto (legacy).
    END IF;
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Variante SIN talla: repone el stock PROPIO de la variante cuando lo lleva;
    -- si no, cae al stock del producto (comportamiento previo).
    IF v_variant_id IS NOT NULL AND v_size IS NULL THEN
      v_variants := v_row.variants; v_var_idx := -1;
      FOR i IN 0 .. COALESCE(jsonb_array_length(v_variants), 0) - 1 LOOP
        IF (v_variants->i->>'id') = v_variant_id THEN v_var_idx := i; EXIT; END IF;
      ...
      IF v_var_idx >= 0
         AND v_variants->v_var_idx->'stock' IS NOT NULL
         AND jsonb_typeof(v_variants->v_var_idx->'stock') <> 'null' THEN
        v_var_stock := (v_variants->v_var_idx->>'stock')::NUMERIC + v_quantity;
        v_variants := jsonb_set(v_variants, ARRAY[v_var_idx::TEXT, 'stock'], to_jsonb(v_var_stock));
        UPDATE products SET variants = v_variants WHERE id = v_product_id;
        v_updated := v_updated + 1;
        CONTINUE;
