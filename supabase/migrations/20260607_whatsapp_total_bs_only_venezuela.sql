-- Fix: el monto en Bs (dual currency) en la notificación de nueva orden sólo
-- debe mostrarse para catálogos de Venezuela. Para otros países, mostrar sólo
-- la moneda configurada del catálogo (sin "(Bs. ...)").
...
BEGIN
  ...
  SELECT t.name, t.country_code,
    ...
    INTO v_name, v_country, v_symbol, v_dual
