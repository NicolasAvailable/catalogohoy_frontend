-- Add country_code to list_paying_clients_admin so the internal "Catálogos
-- activos" table can show each catalog's country. Changing the RETURNS TABLE
-- shape requires dropping the function first.
...
BEGIN
  ...
  WHERE t.plan_id IN ('basico', 'avanzado')
