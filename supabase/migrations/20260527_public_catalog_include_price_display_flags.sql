-- get_public_catalog: include show_reference_price + show_local_currency_price.
--
-- These flags exist in tenant_ecommerce_config (defaulting to true) and are
-- toggled from the catalog editor. The public catalog RPC was not selecting
-- them, so the frontend always fell back to its `?? true` default — meaning
-- the "Mostrar precio en moneda local" toggle worked in the editor preview
-- (which reads the config table directly) but had no effect on the public
-- storefront. This migration adds both flags so the public catalog respects
-- the tenant's configured display preferences.
--
-- Also preserves the out-of-band addition of show_categories_section so the
-- migration file matches the live function definition.
