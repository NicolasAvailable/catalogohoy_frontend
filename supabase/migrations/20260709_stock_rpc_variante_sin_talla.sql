-- Fix: los items con VARIANTE pero SIN talla se saltaban (v_skipped) y nunca
-- movían inventario. Las variantes comparten el stock a nivel de producto
-- (no tienen stock propio; solo sus tallas lo tienen — ver cart.store.ts del
-- frontend), así que una variante sin talla debe caer al stock del producto.
-- Aplica simétricamente a decrement_product_stock e increment_product_stock.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_variant_id IS NOT NULL AND v_size IS NOT NULL THEN
      ...
      CONTINUE;
    END IF;
    -- Variante SIN talla: cae al stock a nivel de producto (compartido).
